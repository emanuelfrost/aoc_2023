...
import Dec3.Companion.STAR
import kotlin.test.assertEquals

fun main(args: Array<String>) {
    val dec3 = Dec3()
    val lines = dec3.input.lines()
    ...
    data class Gear(
        ...
        val top: String,
        val bottom: String,
        val starCoords: List<Coord>
    ){
        private val stringSurroundings = "$left$top$right$bottom"

        val containsSymbols: Boolean
            get() = stringSurroundings.filter { (it.isDigit() || it == '.').not() }.isNotBlank()
    }

    val gears = mutableListOf<Gear>()
    lines.forEachIndexed { currentLineIndex, currentLineString ->
        var tempNumber: String? = null
        var startIndex = -1

        fun addGear(currentLineCharIndex: Int){
            val startAtBeginning = startIndex == 0

            val leftStartIndex = startIndex - 1

            val left = when {
                startAtBeginning -> ""
                else -> currentLineString.substring(leftStartIndex, startIndex)
            }

            val endsAtEnd = currentLineCharIndex == currentLineString.length - 1

            val tempNumberEndIndex = startIndex + tempNumber!!.length

            val right = when {
                endsAtEnd -> ""
                else -> currentLineString.substring(tempNumberEndIndex, tempNumberEndIndex + 1)
            }

            val topBottomStartIndex = when {
                startAtBeginning -> startIndex
                else -> leftStartIndex
            }

            //End index exclusive
            val topBottomEndIndex = when {
                endsAtEnd -> tempNumberEndIndex
                else -> tempNumberEndIndex + 1
            }

            val topLineIndex = currentLineIndex - 1
            val top = when (currentLineIndex) {
                0 -> ""
                else -> {
                    lines[topLineIndex].substring(topBottomStartIndex, topBottomEndIndex)
                }
            }

            val bottomLineIndex = currentLineIndex + 1
            val bottom = when (currentLineIndex) {
                lines.count()-1 -> ""
                else -> {
                    lines[bottomLineIndex].substring(topBottomStartIndex, topBottomEndIndex)
                }
            }

            val starsCoords = mutableListOf<Coord>()

            if(left == STAR.toString()){
                starsCoords.add(Coord(leftStartIndex, currentLineIndex))
            }

            if(right == STAR.toString()){
                starsCoords.add(Coord(tempNumberEndIndex, currentLineIndex))
            }

            top.forEachIndexed{ index, c ->
                if(c == STAR){
                    starsCoords.add(Coord(topBottomStartIndex+index, topLineIndex))
                }
            }

            bottom.forEachIndexed{ index, c ->
                if(c == STAR){
                    starsCoords.add(Coord(topBottomStartIndex+index, bottomLineIndex))
                }
            }
            ...
            gears.add(Gear(
                ...
                top = top,
                bottom = bottom,
                starCoords = starsCoords
            ))
            startIndex = -1
            tempNumber = null
        }

        currentLineString.forEachIndexed { currentLineCharIndex, currentLineChar ->
            val isDigit = currentLineChar.isDigit()
            when {
                tempNumber == null && isDigit -> { //Start
                    startIndex = currentLineCharIndex
                    tempNumber = currentLineChar.toString()
                }
                tempNumber != null && isDigit -> { //Add
                    tempNumber += currentLineChar
                    if(currentLineCharIndex == currentLineString.length-1)
                        addGear(currentLineCharIndex) //End of line
                }
                tempNumber != null && !isDigit -> { //End
                    addGear(currentLineCharIndex)
                }
            }
        }
    }

    val sum = gears.fold(initial = 0L) { acc, gear ->
        if(gear.containsSymbols){
            acc + gear.value.toInt()
        } else {
            acc
        }
    }
    ...
    //First answer
    assertEquals(531561, sum)
    println("$sum")

    //Second answer
    val gearPair = mutableListOf<Pair<Gear, Gear>>()
    gears.forEach { gear ->
        val neighbors = gears.filter { otherGear ->
            when(otherGear == gear){
                true -> false
                else -> otherGear.starCoords.any { otherGearStarCoord ->
                    gear.starCoords.contains(otherGearStarCoord)
                }
            }
        }

        neighbors.forEach{ neighbor ->
            val p1 = Pair(gear, neighbor)
            val p2 = Pair(neighbor, gear)
            if(!gearPair.contains(p1) && !gearPair.contains(p2)){
                gearPair.add(p1)
            }
        }
    }

    val sum2 = gearPair.fold(initial = 0){ acc, pair ->
        acc + (pair.first.value.toInt()*pair.second.value.toInt())
    }

    //Answer 2
    println(sum2)
    assertEquals(83279367, sum2)
}
...
class Dec3 {
    ...
    val input = """................458...689.556..3............197......582........720.........................515..352..286.........670.741.....895.626.......
