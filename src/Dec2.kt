fun main(args: Array<String>) {
    ...
    val games = dec2.input.lines().map {
        val game = it.split(":")

        val rounds = game[1].split(";").map { round ->
            //9 green, 10 red, 2 blue
            val colors = round.split(",")

            val greenCount = dec2.count("green", colors)
            val redCount = dec2.count("red", colors)
            val blueCount = dec2.count("blue", colors)

            Round(redCount, greenCount, blueCount)
        }

        val id: Int = game[0].filter { it.isDigit() }.toInt()
        Game(id = id, rounds)
    }

    //Games with only 12 red cubes, 13 green cubes, and 14 blue cubes
    val validGames = games.filter { game ->
        game.rounds.all { round ->
            round.r <= 12 && round.b <= 14 && round.g <= 13
        }
    }

    val sum = validGames.sumOf { it.id }
    println("Sum of all valid game ids: $sum")

    val gamesFewestNumberOfCubes = games.map {
        val fewestPossibleColors = Round(
            r = it.rounds.maxOf { it.r },
            g = it.rounds.maxOf { it.g },
            b = it.rounds.maxOf { it.b }
        )

        Game(it.id, listOf(fewestPossibleColors))
    }

    val powerOfGamesWithFewestPossibleColors = gamesFewestNumberOfCubes.fold(0) { acc, game ->
        val round = game.rounds[0]
        acc + (round.r * round.g * round.b)
    }

    println("Sum of power of all game minimum dice values: $powerOfGamesWithFewestPossibleColors")
}
...
class Dec2 {
    fun count(color: String, colors: List<String>): Int{
        return colors.fold(initial = 0) { acc, str ->
            if(str.contains(color)){
                val count = str.filter { it.isDigit() }
                acc + count.toInt()
            } else {
                acc
            }
    ...
    val input = """Game 1: 13 green, 3 red; 4 red, 9 green, 4 blue; 9 green, 10 red, 2 blue
